...
namespace BusDriver.Commands
{
		...
		Command
	{
		...
		public bool Execute()
		{
			...
			//.DebugFormat("Using output path name: {0}", fullPath);

			string directoryName = Path.GetDirectoryName(fullPath.GetPath());
			if (!System.IO.Directory.Exists(directoryName))
				System.IO.Directory.CreateDirectory(directoryName);

			IOutboundTransport toTransport = Program.Transports.GetTransport(uri);

			ITextBlock text = new TextBlock()
				.BeginBlock("Load messages to URI: " + uri, "");

			string[] files = System.IO.Directory.GetFiles(directoryName, fullPath.GetName() + "*", SearchOption.TopDirectoryOnly)
				.OrderBy(x => x).ToArray();
